       ...
       const bool is_model_restart_output)
{
  ...
  if (m_is_restarted_run and not m_is_model_restart_output) {
    ...
    if (perform_history_restart) {
      ...
      auto old_storage_type = scorpio::get_global_attribute<std::string>(rhist_file,"file_max_storage_type");
  ...
  if (is_write_step) {
    ...
    auto write_global_data = [&](IOControl& control, IOFileSpecs& filespecs) {
      ...
      if (m_is_model_restart_output) {
        ...
        set_global_attribute(filespecs.filename,"file_max_storage_type",e2str(m_output_file_specs.storage.type));
        if (m_output_file_specs.storage.type==NumSnaps) {
          set_global_attribute(filespecs.filename,"max_snapshots_per_file",m_output_file_specs.storage.max_snapshots_in_file);
        }
      ...
      // We don't need to write the time_bnd var in the hist restart files
      // NOTE: it's not to save space, but rather to avoid errors: if this is not a full checkpoint step,
      //       (e.g., checkpoint coincides with output), we don't add a time frame (no vars to write),
      //       so we can't write the time_bnd var either.
      if (m_time_bnds.size()>0 and filespecs.ftype!=FileType::HistoryRestart) {
            ...
            const IOControl& control)
{
  ...
  if (not m_resume_output_file) {
    ...
    // std::cout << "setting time on file " << filename << "\n";
    define_time(filename,time_units);
    if (use_leap_year()) {
      set_attribute (filename,"time","calendar","gregorian");
    } else {
      set_attribute (filename,"time","calendar","noleap");
    }
    ...
    if (m_avg_type!=OutputAvgType::Instant) {
      ...
      set_attribute(filename,"time_bnds","note","right endpoint accumulation");

      // I'm not sure what's the point of this, but CF conventions seem to require it
      set_attribute (filename,"time","bounds","time_bnds");
    }
    ...
    set_global_attribute(filespecs.filename,"file_max_storage_type",e2str(m_output_file_specs.storage.type));
    if (m_output_file_specs.storage.type==NumSnaps) {
      set_global_attribute(filename,"max_snapshots_per_file",m_output_file_specs.storage.max_snapshots_in_file);
    }
...
void OutputManager::set_file_header(const IOFileSpecs& file_specs)
{
  ...
  set_global_attribute(filename,"case",p.get<std::string>("caseid","NONE"));
  set_global_attribute(filename,"source",std::string("E3SM Atmosphere Model (EAMxx)"));
  set_global_attribute(filename,"eamxx_version",std::string(EAMXX_VERSION));
  set_global_attribute(filename,"git_version",p.get<std::string>("git_version",std::string(EAMXX_GIT_VERSION)));
  set_global_attribute(filename,"hostname",p.get<std::string>("hostname","UNKNOWN"));
  set_global_attribute(filename,"username",p.get<std::string>("username","UNKNOWN"));
  set_global_attribute(filename,"atm_initial_conditions_file",p.get<std::string>("initial_conditions_file","NONE"));
  set_global_attribute(filename,"topography_file",p.get<std::string>("topography_file","NONE"));
  set_global_attribute(filename,"contact",std::string("e3sm-data-support@llnl.gov"));
  set_global_attribute(filename,"institution_id",std::string("E3SM-Projet"));
  set_global_attribute(filename,"realm",std::string("atmos"));
  set_global_attribute(filename,"history",ts_str);
  set_global_attribute(filename,"Conventions",std::string("CF-1.8"));
  set_global_attribute(filename,"product",e2str(file_specs.ftype));
}
/*===============================================================================================*/
void OutputManager::
push_to_logger()
